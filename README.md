...
## 1. 진행 상황 🛠️
- 결제 완료 후 결제 완료된 데이터와 결제 취소를 할 때 데이터 처리를 어떻게 해야 할지 고민하던 중 2가지 방식을 찾았다.
  1. paymemt 테이블에 status를 사용하여 결제 완료, 취소 데이터 모두를 저장하는 방법
  2. paymentComplete, paymentCancel 테이블을 구분해서 만드는 방법
- 찾아본 결과 2번은 금융권에서 많이 사용하는 방법이므로 1번이 더 적합하다고 생각해서
  기존 status를 사용하고 취소에 관한 추가적인 컬럼을 추가하기로 했다.

---

- **프로젝트/기능명**: [payment 엔티티 수정]
- **진행한 작업**:
    - [O] 기능 개발: 
    - CanceledAt, canceledReason 컬럼 추가 
    - CanceledReason을 Enum 타입으로 설정하여 간단 명료하게 저장하게 사용할 예정이다.
