...
- 화면에 출력까지 성공했다.
### 고민사항
1. 지금은 `totalPrice`에 대한 고민중이다. 화면에서 주문할때 `price * count`를 해서 `totalPrice`를 보여주는데
- 이미 합계 계산을 했는데 데이터베이스에 넣을때 `합계 로직`을 구현할 필요가 없을꺼 같아서 안했는데
- 기능 분리를 위해 만들어야 할꺼 같다는 생각이 든다.
2. 화면에 `totalPrice`를 보여주고싶은데 `하나의 Order`에 `여러개의 OrderItem`이 있을 수도 있어서 
- 그 데이터베이스 안에 있는 전체 `OrderItem`의 `totalPrice`를 서비스에 합쳐서 전송할려고 구현중인데 어렵다
